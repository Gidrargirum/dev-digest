...
pnpm typecheck
pnpm arch:check           # Onion ring boundaries; fails on any NEW violation
```
...
- `src/adapters/` — adapter *implementations* (llm · github · git · codeindex ·
  secrets · tokenizer · depgraph) plus `mocks.ts` for tests.
...
- `src/ports/` — port interfaces the client has no use for (`Tokenizer`,
  `DepGraph`). Same innermost ring as `vendor/shared`, but not vendored.
...
- Layering is Onion and is **enforced**, not advisory: `.dependency-cruiser.cjs`
  + `pnpm arch:check`. For which ring a file belongs to, invoke the
...
- `pnpm test:unit` / `pnpm test:integration` do not exist and never have — the
  lane split lives in the command, not in a script. Call `pnpm exec vitest run …`
  (see Commands above).
...
- `package.json` — normally editable, but check first: `git ls-files -v
  package.json` (`S` = `skip-worktree`, edits won't commit · `H` = normal).
  The flag is per-clone local state, never committed, so it can differ from
