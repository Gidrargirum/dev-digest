...
pnpm typecheck
pnpm lint         # eslint --max-warnings 0; there is no suppression baseline
pnpm build
```
...
- Import via the `@/` alias once the relative path would reach 4 levels up.
  Siblings and near-neighbours stay relative. Linted.
- `src/components/` and `src/lib/` are the shared layer: they must not import
  from `src/app/`. The reverse is the intended direction. Linted.
