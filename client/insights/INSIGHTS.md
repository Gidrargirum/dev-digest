...
- **2026-08-27** — `RunTraceDrawer`'s `TraceSection` (`_components/TraceSection/TraceSection.tsx`) mounts `defaultOpen={false}` for "Prompt assembly": its `PromptBlock` children (and their labels, e.g. `trace.prompt.specs`) don't exist in the DOM until the section header itself is clicked once — `getByText` on a prompt-block label fails with a generic "could not find" error that gives no hint the parent section is collapsed.
- **2026-08-27** — RTL's default whitespace normalizer collapses newlines, so `getByText` on a multi-line `<pre>` block (e.g. `PromptBlock`'s expanded body) never matches the literal source string even when it's rendered verbatim. Use a matcher function comparing `node.textContent` directly (`(_, node) => node?.textContent === expected`) instead of a normalizer option, to assert on exact untruncated block content.
