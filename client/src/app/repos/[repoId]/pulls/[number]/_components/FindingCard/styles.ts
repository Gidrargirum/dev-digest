...
export const s = {
  card: (focused: boolean, sevColor: string, muted: boolean): CSSProperties => ({
    borderRadius: 8,
    // Per-side longhand throughout. `borderColor`/`borderWidth` are themselves
    // shorthands for all four sides, so pairing them with `borderLeft*` made
    // React warn on every rerender that changed `focused` — which is exactly
    // what the finding deep-link does.
    borderStyle: "solid",
