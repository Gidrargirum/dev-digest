 ...
 * set — see the disabled state on FindingCard's action (AC-4).
 */
export function buildEvalSeed(f: FindingRecord): EvalCaseSeed | null {
  if (f.accepted_at) {
    return {
  ...
  if (f.dismissed_at) {
    return { expectation_type: "must_not_flag", expected_output: [], name: f.title };
  }
  return null;
}
