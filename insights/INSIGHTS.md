...
- **2026-08-29** — `evals/README.md:77-88` is stale about OpenRouter's Anthropic Messages endpoint being limited to `anthropic/*`: OpenRouter now documents multi-model `POST /api/v1/messages` with tools, so `google/gemini-2.5-flash` may no longer require the LiteLLM bridge. Keep the measured LiteLLM route for CI until a key-backed Claude Agent SDK → OpenRouter → Gemini smoke test proves direct streaming, project settings, tool traces, and subagent dispatch end to end.
