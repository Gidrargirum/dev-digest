...
- `--text` matches **rendered** text, so assert what an uppercase-styled element
  actually shows (`FINDINGS`, not the `"Findings"` i18n string).
- `wait --url` only means the route changed. Before clicking a row, `wait --text`
  for that row — otherwise the click races the fetch.
- Open a `HoverCard` with `hover`, never `click`: the driver focuses first, which
  already opens it, and the trigger's `onClick` then toggles it shut.
