...
- **No root `package.json`.** "Root tooling" = the dev-tooling dependencies
  ...
  dependency-cruiser) plus anything under `scripts/` and `.claude/hooks/`. Report
...
- **Discovery, never a hardcoded package list.** `evals/` exists and CLAUDE.md
