...
- **Routing self-check**: compare `ls -d .Codex/skills/*/` against the skills
  named in [routing.md](routing.md). A skill on disk with no routing rule will
  never run and nobody will ever notice — report it as HIGH. (`.Codex/skills/README.md`
...
Update `.Codex/pr-self-review/gates-receipt.json` with whatever you end up
...
```sh
node .Codex/hooks/pr-self-review-gate.mjs --fingerprint   # → "<head> <worktreeHash>"
```
...
`.Codex/pr-self-review/receipt.json` (git-ignored).

### 7. PR body

Only on PASS or OVERRIDE. Write `.Codex/pr-self-review/pr-body.md` from the
...
.Codex/pr-self-review/pr-body.md` then picks it up.
